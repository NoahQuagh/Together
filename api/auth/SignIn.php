...
if (!$prenom) $erreurs[] = __tphp('the first name is mandatory') . 'auth';
if (!$nom) $erreurs[] = __tphp('the name is mandatory') . 'auth';
if (!$email || !filter_var($email, FILTER_VALIDATE_EMAIL))
    $erreurs[] = __tphp('invalid email address') . 'auth';
if (strlen($mdp) < 8) $erreurs[] = __tphp('the password must be at least 8 characters long') . 'auth';
if ($mdp !== $mdp2) $erreurs[] = __tphp('the passwords do not match') . 'auth';
if (!$cgu) $erreurs[] = __tphp('you must accept the Terms of Use') . 'auth';
